...
$query = "update gclWish set fk_userID = ?, wish=?, priority=?, wishDesc=?, weblink=?,bought =?";
$type='isissi'; 

    //create parameter array
$params= array(&$fk_userID, &$item, &$priority, &$wishDesc,&$link,&$purchased);
    //run query
$wishId = $db->query_param($query,$type,$params);
...
<div>
  ...
  <input type="hidden" name="MAX_FILE_SIZE" value ="<?php echo "http://loki.trentu.ca/~bchristyanton/www_data/".$row["filepath"]; ?>">
...
<img src='<?php echo "http://loki.trentu.ca/~bchristyanton/www_data/".$row["filepath"]; ?>' alt=""/>
